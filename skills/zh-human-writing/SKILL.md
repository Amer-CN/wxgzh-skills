...
- **文件读写**：仅限调用方显式传入的文本/文章文件与同目录校验报告（diff/audit 输出）。
- **网络访问**：无。**凭据**：无。
- **子进程**：verify_release.py 与 tests/run_tests.py 以 sys.executable 本地自检本技能脚本。
- **明确不做**：不改事实/数字/URL（fidelity 保护，最高优先级）；不安装依赖；不做任何发布动作。
