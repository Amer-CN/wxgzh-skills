...
def test_77w_single_source_guard_media_skill_boundary():
    """守卫:media SKILL.md「自动决策边界」节含「必须人工:图片审批(默认道)」
    ——合同镜像与真源两文一致才绿。"""
    text = (SKILL_ROOT.parent / "media-enrichment" / "SKILL.md").read_text(
        encoding="utf-8")
    assert "### 自动决策边界" in text
    assert "**必须人工**：图片审批（默认道）" in text
    assert "auto_approve 开启（WXGZH_MEDIA_AUTO_APPROVE=1，默认关）" in text
