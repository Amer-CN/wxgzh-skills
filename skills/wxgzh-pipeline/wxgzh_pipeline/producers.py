...
def _build_media_request(ctx, sd: Path, state, *, phase: str = "discover") -> Path:
    ...
    req = {
        ...
        "config": {
            ...
            # 76R/OBS-289:媒体审批自动放行模式(默认关;开启=单图证据链齐全自动批准)
            "auto_approve": ctx_env.get("WXGZH_MEDIA_AUTO_APPROVE", "0").strip().lower() in ("1", "true", "yes"),
...
def _entry_args(
    ...
    if stage == "gzh_design":
        ...
        if cover:
            for flag, key in (("--strike", "strike"),
                              ("--strike-assumption", "strike_assumption"),
                              ("--tags", "tags"),
