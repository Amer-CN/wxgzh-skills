...
- **关键词实证表**：并入 title-patterns.md——正向 GitHub/Star 3.84×、Skill 3.23×、一键/直接用 3.16×；饱和/负向 开源 1.02×、Agent 0.93×、DeepSeek 0.30×（2025-07 后失效，关键词价值会过期）、免费/白嫖 0.26×；四任务规则+禁堆叠只描述技术无读者结果的词。数据来自 liucongg/liucong-skills empirical-signals.md（Apache-2.0，77D 先例：引用数据注明出处，不搬运原文段落）。
- **网感组配方化**：title-playbook.md §2 网感点击组从一句话升级为 §2.1 四步配方（①双目标二选一 ②六范式选型 ③关键词正负向选词 ④结构约束）；§6 追加关键词四任务与禁堆叠纪律。其余各组、五维评分、风险标记、推荐结构、候选总数 3–5 不动（77D/77O/77Z 语义零回退）。
- **hits 台账**：新增 `references/title-hits.md`——schema（日期/RUN_ID/标题/组/五维分/风险标记/表现回填位/验证状态）+ 三态验证（待回填/已验证/已证伪）+ 纪律（只追加不删改/只收本号真实数据，不抄别家防基数污染）+ 首行种子（2026-09-07/3bhpi2/网感点击/22 分 5·4·5·4·4/无据-用户裁决接受/待回填）。sw Phase 6 指令面追加发文后回填义务（77AA/OBS-377，轻义务不阻断流水线）。
- **测试 +7**：新增 tests/test_hf77aa_title_patterns.py（六范式名/禁用词/关键词正负向/playbook 配方锚点/hits schema 三态与种子/原榜标题零命中/sw 指令义务与 3000 上限）。
...
- **OBS-306 预算对齐链路**:align_outline_budget 新增 --article 参数——按正文各节
  实际可见字数（## 分节、去空白）重排预算,不再只按 planned 估计缩放;actual 缺失或
  全部为零时回退 76V 证据加权/原比例语义;测试 +2（actual 反向时预算跟实际/无
  article 回退）。
