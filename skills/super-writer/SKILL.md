...
**标题打磨（档76A 新增，攻核之后）：** 产出 3–5 个候选标题（`title_candidates`）与一句钩子（`hook_line`，一句话说清本文最反直觉/最值钱的点），候选须覆盖不同角度。原则服从 R1–R9：标题要具体、有判断，禁标题党空壳；hook_line 仅供导读段（副标题兜底）；划线句用 formatter.cover.strike_assumption（被本文证据否定的旧认知，≤40 字，须素材可支撑、禁稻草人；缺失则渲染端划线句整行不渲染）。产物写入 handoff.yaml（v2.2）。最终 H1 由 Phase 6「标题选定」从候选中选定（档76B）。
