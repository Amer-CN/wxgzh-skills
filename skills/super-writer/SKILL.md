...
- **文件读写**：仅限调用方显式传入的文章/素材/校验产物路径（项目 RUN 目录与 .temp 工作区）；不扫描用户主目录，不改系统与全局配置。
- **网络访问**：无。本技能不含任何网络调用。
- **凭据**：不读取任何凭据或密钥。
- **子进程**：生产脚本无 subprocess；tests/ 下测试用 sys.executable 调用本技能内脚本自检，命令为仓库内固定路径，不拼接外部输入。
- **明确不做**：不安装/升级依赖，不执行 npm/pip，不做正式发布/群发/定时发布，不删除用户文件。
...
**标题打磨（档76A 新增，攻核之后）：** 产出 3–5 个候选标题（`title_candidates`）与一句钩子（`hook_line`，一句话说清本文最反直觉/最值钱的点），候选须覆盖不同角度。原则服从 R1–R9：标题要具体、有判断，禁标题党空壳；hook_line 仅供导读段（副标题兜底，77P 单行预算 20）；划线句用 formatter.cover.strike_assumption（被本文证据否定的旧认知，77P 单行预算 18，CJK=1/ASCII=0.5，须素材可支撑、禁稻草人；缺失则渲染端划线句整行不渲染）。产物写入 handoff.yaml（v2.2）。最终 H1 由 Phase 6「标题选定」从候选中选定（档76B）。
