...
- **题材有明确契合** → 单问确认：「建议用 XX（理由），确认还是换一套？」选项给推荐项 + 1~2 个备选 + "看全部"。题材→主题契合参考 theme-index"适用场景"列：
  ...
  - 内刊手记 / 深度评测 / 案例复盘 → 橄榄手记
- 产品发布 / 科技评论 / 品牌故事 / 工具测评 / 有态度但不适合红白强烈风格的观点文章 → 锤子风格
...
(3) **如果源稿含 `:::` 围栏语法或 `[^N]` 脚注**，Read [references/advanced-components.md](references/advanced-components.md)（高级组件总目录）和 [references/advanced/theme-adapters.md](references/advanced/theme-adapters.md)（主题适配器），按当前主题取高级组件 HTML 模板。高级组件的 `:::` 语法是标准 Markdown 的严格超集——不含 `:::` 的文章行为与升级前完全一致。

### 2.5 高级组件语义扫描与计划（新增）

在解析 Markdown 结构之前，先执行高级组件语义扫描：

1. **扫描 `:::` 围栏语法**：`:::alert`、`:::quote`、`:::code-compare`、`:::media-text`、`:::gallery`、`:::long-image`、`:::resources`、`:::dialogue`、`:::facts`、`:::decision`、`:::steps`、`:::compare`、`:::annotated-image`、`:::faq`、`:::timeline`、`:::checklist`、`:::case`、`:::cta`
2. **扫描 `[^N]` 脚注**：正文中 `[^1]` 引用 + 文末 `[^1]:` 定义
3. **自动识别**（仅在源稿已有完整事实/条件/链接/图片/结构时）：
   - 源稿有明确风险提示语义 → 可升级为 alert（需有完整正文）
   - 源稿有 2+ 个 HTTPS 链接且语义为参考资料 → 可升级为 resources
   - 源稿有 2-4 张相关图片 → 可升级为 gallery
   - 源稿有明确的改前/改后代码对照 → 可升级为 code-compare
4. **内部组件计划表**：列出本文将使用的高级组件，默认 3-6 个；短资讯 0-2 个
5. **降级规则**：
   - 无图片 → 不得生成 gallery/long-image/media-text
   - 只有 1 个普通链接 → 使用原版链接，不生成 resources
   - 单一代码块 → 使用通用库 1a/1b，不生成 code-compare
   - 无 `[^N]` 脚注 → 不生成脚注区
   - 缺少高级模块字段 → 回退到原版组件或正文，**绝不保留占位符**
6. **兼容性铁律**：`preserve_exactly`、`[[protected]]`、`<!--keep-->` 内容逐字保留；代码块/URL/数字/日期逐字保留
7. **编辑锚点阻断规则**：
   - 草稿 Markdown / handoff 中允许保留 `editor_anchors`、`unresolved_facts`，供作者继续编辑
   - **可发布 HTML 必须阻断**：若源稿中存在 `[编辑锚点：...]`、`TODO`、`待补`、`需要补充`、占位符（`{{...}}`），必须：
     - 阻止生成 HTML 或标记为不可发布
     - 不得将这些标记渲染进公众号正文
     - 提示作者先补完内容再排版
8. **组件审计**（生成 HTML 后、校验前）：
   - 检查是否有占位符残留（编辑锚点/TODO/待补/`{{...}}`）
   - 检查相邻两个大容器之间是否有正常正文缓冲
   - 检查全组件样稿只用于验收，真实文章默认 3-6 个组件、短资讯 0-2 个
   - 检查无图片时不得生成媒体组件
   - 检查组件选择与源稿语义匹配
9. **发布路径门禁**（最终 release HTML 必须通过）：
   - 不得包含 `../assets/`、`file://`、本地磁盘路径（如 `C:\` 或 `/Users/`）
   - 所有图片 `src` 必须是作者提供的可访问 HTTPS URL 或公众号图床 URL
   - 本地测试图片（`../assets/*.png`）仅用于本地预览，不得进入正式发布 HTML
   - 若发现本地路径残留 → 标记为不可发布，提示作者替换为可访问图片 URL
...
6. **开头引言卡署名**：按文章的**作者或主题**而定——文章有署名就写"—— 作者名"，没有明确作者就用与主题相关的简短落款或直接省略。**不要固定写"甲木"**。
7. **固定结尾署名组件（全局文章尾部组件，所有主题通用，仅末尾一处）**：所有 7 个主题都必须在文章末尾使用 `common-components.md` 第四部分的固定结尾署名组件，**不再生成 `{{作者名}}` / `{{一句话简介}}` 等动态占位符**。
   - **固定文案（逐字一致，不得改写、扩写、润色）**：
     ```
     好了，今天就先聊到这儿。
     ...
     ```
   - "AI"保持大写；邮箱 `cd.hyxc.jz@foxmail.com` 保持半角原样；`/` 保持半角；不得改成"我是……"；不得生成其他作者名；不得出现 `{{作者名}}` 或 `{{一句话简介}}`。
   - **固定作者**：给自己造把锤子。**固定投稿邮箱**：cd.hyxc.jz@foxmail.com。
   - **插入位置**（文章最终顺序）：文章正文 → 文章自身的总结/结语 → **固定结尾署名组件**（只出现一次）→ 点赞/在看/转发视觉区域（主题存在时）→ THANKS FOR READING（主题存在时）。
   - **视觉**：文案固定，视觉随主题适配（三层结构：收尾句用正文色正常字号；固定品牌句用当前主题主色做轻强调——左竖线/浅底/下划线，不要大型 CTA；作者与联系方式用次要文字色 12～13px）。具体换色映射见 `common-components.md` 4b。
   - **去重**：源稿已完整包含这段固定结尾 → 不重复追加，保留已有一份，统一转换为当前主题的结尾组件样式；源稿只有普通作者署名 → 不把旧署名与固定署名同时输出；原文明确要求保留其他作者署名时，**停止并提示存在署名冲突**，不得静默篡改真正的外部投稿作者。
   - **邮箱链接限制**：邮箱使用纯文本；如要使用链接，**只允许** `mailto:cd.hyxc.jz@foxmail.com`，不得修改邮箱字符。
...
用户明确要求发布时才执行，使用本次生成并校验通过的 HTML：

```bash
python <SKILL_ROOT>/scripts/publish_wechat_draft.py \
  --html "<本次生成的 article.wechat.html>" \
  --title "<文章标题>" \
  --thumb-media-id "<封面素材ID>"
```

需设置环境变量 `WECHAT_APP_ID` 和 `WECHAT_APP_SECRET`。
成功后返回 `media_id`，提醒用户到公众号后台草稿箱检查。默认只创建草稿，不正式发布和群发。
