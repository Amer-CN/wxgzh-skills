# gzh-design v2026.09.14-hammer.25

## v2026.09.14-hammer.25(档77AJ)
- toc 卡片标题去序号显示：`hammer_toc` 内标题显示副本脱序号（`一、` 等中文数字 / `1.` 等阿拉伯数字两式，脱空回退原文；PART /// 末卡不动）；副标题/章节数据/正文/锚点不动。显示裁剪，palette/字号/配色零触碰；SHA256SUMS 更新。
- 回归测试 +1：tests/test_hf77aj_toc_denum.py（带序号脱号+单行/无序号原样/脱空回退/章节体仍带编号）。
