...
| 227 | SC-005 特殊逻辑量错了对象:不只是双重计数(consecutive_count>=3 逐句 append,5 句产出 3 条)与缓慢漂移误判(20→25→30→35 判同构),而是**实测的是句长差(<=5),不是文档声明的「相同句式」**;规则名已由 72C-2 改为「连续等长句」以名实对齐 | 未修(严重性上调:语义重写=句式同构检测留 Batch 3;双重计数/跨段分句同批处理;72C-6R 实证见 238:分段后条数不变,split_sentences 不读段落边界) | zh-human-writing scripts/pattern_audit.py::detect_strong_contextual(SC-005) | 72B-2R/72C-2 |
...
| 251 | 封面文案逐篇定制路径缺失:门禁封死手工定制后无替代通道(用户五张历史封面截图实证),封面/署名文案只能落规范默认值;HF-6 已备渲染参数机制(--date/--strike/--brand/--tags)但写作侧尚无结构化产出与 handoff 契约流入 | 未修(修复方向=封面文案成为写作侧结构化产出并经 handoff 契约流入渲染参数;归 Batch 3 契约设计) | gzh-design render_article.py 参数机制(HF-6);写作侧 handoff 契约(未建) | HF-7 |
...
| 251 | 封面文案逐篇定制路径缺失(门禁封死手工定制后无替代通道,用户五张历史封面截图实证) | 未修(修复方向=写作侧结构化产出+handoff 契约流入渲染参数,HF-6 已备参数机制;归 Batch 3 契约设计) | 不阻塞(默认封面可正常出稿,定制属增强需求) |
