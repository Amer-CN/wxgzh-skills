...
| 211 | runtime_manifest_sha256 不提供 required_files 内容哈希(0A-1/0A-2 实证:该值=compute_runtime_manifest_sha 的运行时文件【清单】哈希,skill_discovery.py:67,relock 写锁为重算赋值 relock.py:567/622;0A-3 三组探测 root 变/manifest 不变,四 skill 行为一致);72A-F「形同虚设」结论修正为「口径澄清,内容由 skill_root_sha256 覆盖」;但 required_files 单文件内容漂移无独立指标;72C-3 猜想入档:连续四次 relock(72B-2R/72B-2F/72C-2/72C-3)对 required_files 内文件内容变化均不响应,疑为只哈希文件名清单(compute_runtime_manifest_sha 的输入是 relpath 列表),Batch 3 用 grep skill_discovery.py 证实或证伪 | 未修(留升级机制档决定是否新增 required_files 内容哈希指标;当前内容完整性由 root sha 覆盖) | wxgzh_pipeline/skill_discovery.py:67;scripts/relock.py:567/622 | 72B-0 |
