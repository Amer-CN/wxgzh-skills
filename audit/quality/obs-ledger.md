...
42. 76B(标题选定闭环,用户批准 2026-08-10,RELOCK_ALLOWED 临时 0→1 范围本档):Phase 6 内容审稿新增子步骤「标题选定」——Reviewer 角色(非 Writer)按固定评分尺(具体>有判断>贴核心张力>长度≤30字>无标题党空壳)从 title_candidates 选定最终 H1 并写一行理由;article.md H1=选定标题(封面拆行随 H1 自动生效);title_candidates 保留备查;禁止另造候选外新标题。handoff v2.2 追加 selected_title/title_selection_reason 两字段(字符串)。Phase 3.5 描述更新。测试 +1(字段存在性/类型/契约),242 passed。无新 OBS。sw 0.3.6-rc1(relock 待网络恢复后执行;commit 9ab1bce 已落盘待推)。
